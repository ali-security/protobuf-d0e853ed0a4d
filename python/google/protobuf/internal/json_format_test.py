...
class JsonFormatTest(JsonFormatBase):
  ...
  def testListValueRecursionDepthEnforcement(self):
    """Test that nested ListValue messages respect max_recursion_depth limit."""
    message = struct_pb2.ListValue()
    # Each nesting level costs two depth units, because it is
    ...
    # This should pass as it only reaches depth 5.
    shallow_list = [[[]]]
    json_format.ParseDict(shallow_list, message, max_recursion_depth=5)
  ...
  def testAnyRecursionDepthBoundary(self):
    ...
    # Create nested Any at depth exactly 4, which succeeds with
    # max_recursion_depth=5.
    depth_4_any = {
        '@type': 'type.googleapis.com/google.protobuf.Any',
        'value': {
    ...
    # Create nested Any at depth exactly 5, which fails with
    # max_recursion_depth=5.
    depth_5_any = {
        '@type': 'type.googleapis.com/google.protobuf.Any',
        'value': {
